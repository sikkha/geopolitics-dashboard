...
const dashboardData = {
    ...
    strategies: {
        ...
        items: [
            ...
            {
                id: 5,
                title: "Launch a Thematic Macro Fund / Signal Feed",
                signal: "You're already modeling this more deeply than most macro hedge funds.",
                optionsTitle: "Options:",
                actions: [
                    "📡 Start a private signal feed for family offices or high-net-worth followers.",
                    "📘 Write a contrarian report (like Palantir + Dalio style) and distribute to asset allocators.",
                    "💼 Launch a stealth thesis-driven fund or SPV: build strategy, issue thesis deck, raise from believers."
                ]
            }
        ],
    }
...
// Added responsive padding and font size adjustments
...
const TabbedCard = ({ section, imageURL, activeTab, setActiveTab }) => {
    // Tab Styles with responsive adjustments
    const tabBaseStyle = "px-3 py-1 text-xs sm:px-4 sm:py-2 sm:text-sm font-medium rounded-t-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 dark:focus:ring-offset-gray-800 transition-colors duration-150 ease-in-out whitespace-nowrap"; // Added whitespace-nowrap
    ...
    return (
        <div key={section.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md mb-4 md:mb-6 flex flex-col overflow-hidden"> {/* Added overflow-hidden */}
            {/* Card Header */}
            <div className="p-3 md:p-4 lg:p-6 border-b border-gray-200 dark:border-gray-700">
                <h2 className="text-lg md:text-xl font-semibold text-gray-800 dark:text-gray-200 flex items-center">
            ...
            {/* Tab Buttons */}
            {/* Added flex-wrap to allow tabs to wrap on very small screens if needed */}
            <div className="border-b border-gray-200 dark:border-gray-700 px-3 md:px-4 lg:px-6">
                 {/* Use overflow-x-auto for horizontal scroll on small screens if wrapping is not desired */}
                                ...
                                {section.assets.map((asset) => (
                                    <li key={asset.name} className="border-b border-gray-200 dark:border-gray-700 pb-2 last:border-b-0">
                                        <div className="flex justify-between items-start gap-2"> {/* Added gap */}
                                            ...
                                            <span className={`font-medium text-xs sm:text-sm text-right ${getTrendIndicator(asset.trend)}`}> {/* Added text-right */}
                                                {asset.signal}
                                            </span>
                                        </div>
                ...
                {activeTab === 'chart' && (
                    <div>
                        <img
                            ...
                            className="w-full h-auto max-w-full rounded-md shadow-sm mb-3 md:mb-4" // Ensured max-w-full
...
function GeopoliticalDashboard() {
    ...
    // State for active tabs for each matrix
    ...
    // Helper to get the right state/setter/URL based on section ID
    ...
    return (
        // Adjusted main padding for mobile
        <div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-2 sm:p-4 md:p-8 font-sans">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                ...
                <Card title={dashboardData.assessment.title} icon={dashboardData.assessment.icon}>
                 ...
                 {/* Strategies Section */}
                 {/* Added responsive adjustments here too */}
...
// ... (paste all the code from the Canvas here) ...
...
// export default App; // Ensure App is exported if it wasn't already
